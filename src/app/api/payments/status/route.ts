...
export async function GET(request: Request) {
  try {
    ...
    const order = await prisma.paymentOrder.findFirst({ where: { id, userId: user.id }, select: { id: true, kind: true, status: true, amountKopecks: true, creditedAt: true, createdAt: true } });
    if (!order) return NextResponse.json({ error: 'Платёж не найден' }, { status: 404 });
    return NextResponse.json({ ...order, amount: kopecksToRubles(order.amountKopecks), amountKopecks: undefined });
  } catch (error) {
